...
const app = express();
const PORT = 3000; // Un solo puerto
  ...
  try {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      messageContent.push({
        ...
        source: {
          type: 'base64',
          media_type: mimeType,  // ✅ Usar MIME type correcto
          data: cleanBase64
        }
      });
    }
...
// Servir frontend para todas las rutas (SPA)
app.get('*', (req, res) => {
  res.sendFile(path.join(__dirname, 'dist', 'index.html'));
});

app.listen(PORT, () => {
  console.log(`🚀 MOGA Studio running on http://localhost:${PORT}`);
  console.log(`📸 Image analysis endpoint: POST /api/analyze-media`);
  console.log(`✅ Frontend + Backend en UN servidor`);
});
